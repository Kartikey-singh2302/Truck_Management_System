...
public class BidServiceImpl implements BidService {
    ...
    @Override
    public List<Bid> BestBids(UUID loadId) {

        List<Bid> bids = bidRepository.findByLoadId(loadId);

        bids.forEach(bid -> {
            double score = (1 / bid.getProposedRate()) * 0.7
                    + (bid.getRating() / 5) * 0.3;
            bid.setScore(score);
        });
    ...
    public List<Bid> getBidsByFilter(UUID loadId, UUID transporterId, BidStatus status) {
        if (loadId != null && transporterId != null && status != null) {
            return bidRepository.findByLoadIdAndTransporterIdAndBidStatus(loadId, transporterId, status);
            ...
            return bidRepository.findByLoadIdAndBidStatus(loadId, status);
        } else if (transporterId != null && status != null) {
            return bidRepository.findByTransporterIdAndBidStatus(transporterId, status);
        } else if (loadId != null) {
            return bidRepository.findByLoadId(loadId);
        } else if (transporterId != null) {
            return bidRepository.findByTransporterId(transporterId);
        } else if (status != null) {
            return bidRepository.findByBidStatus(status);
        } else {
